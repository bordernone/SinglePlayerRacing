...
It is really hard to decisively divide the contributions among ourselves since we worked together and helped each other debug many problems that came up. Thus, I will describe the parts that I spent the most time on. One of them that I focused on was setting up *neDB* to store and retrieve players' data. Players' scores and names are stored when they finished their races. Since only the top 10 players' data is stored in our database, I needed to sort, add, and remove new players' data. 
